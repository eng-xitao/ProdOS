    ...
    <div className="stock-page">
      ...
      <header style={styles.header}>
        <div>
          ...
          <p style={styles.subtitle}>Controle de produto acabado com estoque mínimo, ponto de pedido, estoque máximo e classificação ABC.</p>
        </div>
      ...
      <div className="print-only" style={styles.printHeader}>
        <strong>ProdOS</strong><br />
        <span>Relatório de Estoque — Produto Acabado</span><br />
      ...
      {loading ? <p style={styles.dim}>Carregando...</p> : products.length === 0 ? <p style={styles.dim}>Nenhum produto acabado ativo cadastrado.</p> : (
        <div className="stock-table" style={styles.tableWrap}>
          <table style={styles.table}>
            <thead><tr>
              <th style={styles.th}>SKU</th><th style={styles.th}>Produto</th><th style={styles.th}>Disponível</th>
              <th style={styles.th}>Estoque mínimo</th><th style={styles.th}>Ponto de pedido</th><th style={styles.th}>Estoque máximo</th>
              <th style={styles.th}>Curva ABC</th><th style={styles.th}>Situação</th>
            </tr></thead>
            <tbody>{products.map((p) => {
              ...
              return <tr key={p.id}>
                <td style={styles.td}>{p.sku}</td><td style={styles.td}>{p.name}</td>
                ...
                <td style={styles.td}><span style={{ ...styles.badge, ...situationStyle(situation) }}>{situation === "zerado" ? "Zerado" : situation === "baixo" ? "Abaixo do mínimo" : "OK"}</span></td>
              </tr>;
            })}</tbody>
          </table>
        </div>
