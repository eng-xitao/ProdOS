...
export default function CRMPage() {
  ...
  async function createOpportunity(data) {
    const { data: row, error: e } = await supabase.from("opportunities").insert({
      ...
      status: "aberta",
    }).select("id, opportunity_number, title, customer_id, stage_id, estimated_value, expected_close_date, status, created_at, owner_profile_id, source, priority, customers:customer_id(name,condicao_pagamento)").single();
    if (e) throw e;
    await load();
    setShowNew(false);
...
function CRMDrawer({ opportunity:o, stages, customers, products, company, profile, onClose, onRefresh, onChange, navigate }) {
  ...
  async function loadDetails() {
    const [a,it,sh,q] = await Promise.all([
      ...
      supabase.from("quotes").select("id,code,status").eq("opportunity_id",o.id).maybeSingle(),
    ]);
    setActivities(a.data||[]); setItems(it.data||[]); setStageHistory(sh.data||[]);
    setLinkedQuote(q.data||null);
    if (q.data) {
      const { data: order } = await supabase.from("sales_orders").select("id,code,status").eq("quote_id",q.data.id).maybeSingle();
      setLinkedOrder(order||null);
    } else setLinkedOrder(null);
  }
  ...
  return <div style={S.overlay}><aside style={S.drawer}>
    ...
    <div style={S.stepper}>{stages.map((s,i)=><button key={s.id} disabled={saving || o.status!=="aberta"} onClick={()=>moveStage(s.id)} style={{...S.step,...(i===current?S.stepCurrent:{}),...(i<current?S.stepDone:{}),...(o.status!=="aberta"?S.stepLocked:{})}}><span>{i<current?"✓":i+1}</span>{s.name}</button>)}</div>
    {o.status!=="aberta" && <div style={{...S.nextBox,...S.closedBanner}}><strong>{o.status==="ganha"?"✓ Venda ganha":"× Venda perdida"}</strong><span>Essa oportunidade está encerrada — a etapa não muda mais.</span></div>}
    {o.status==="aberta" && <div style={S.nextBox}><strong>Próximo passo</strong><span>{qualification.next || "Ainda não definido"}</span>{qualification.nextDate && <small>{formatDate(qualification.nextDate)}</small>}<button onClick={()=>setTab("qualificacao")} style={S.linkBtn}>Definir próximo passo →</button></div>}
    ...
    {tab === "resumo" && <div style={S.body}>
      ...
      {o.status==="aberta" && <section style={S.section}><h3>O que fazer agora</h3><p style={S.help}>Use os botões abaixo. O CRM registra automaticamente a etapa e o histórico.</p><div style={S.bigActions}><button style={S.green} onClick={()=>setStatus("ganha")} disabled={saving}>✓ Marcar como ganha</button><button style={S.danger} onClick={()=>setStatus("perdida")} disabled={saving}>× Marcar como perdida</button></div></section>}
      ...
      {o.status==="ganha" && (
        <section style={S.section}>
          <h3>Pós-venda</h3>
          <div style={S.postSaleRow}>
            <div><strong>Orçamento</strong><span style={S.postSaleStatus}>{linkedQuote ? `${linkedQuote.code} — ${linkedQuote.status}` : "Ainda não gerado"}</span></div>
            {linkedQuote && <button style={S.linkBtnBox} onClick={()=>navigate(`/orcamentos?abrir=${linkedQuote.id}`)} type="button">Ver orçamento →</button>}
          </div>
          <div style={S.postSaleRow}>
            <div><strong>Pedido de Venda</strong><span style={S.postSaleStatus}>{linkedOrder ? `${linkedOrder.code} — ${linkedOrder.status}` : "Aguardando aprovação do orçamento"}</span></div>
            {linkedOrder && <button style={S.linkBtnBox} onClick={()=>navigate(`/pedidos-venda?abrir=${linkedOrder.id}`)} type="button">Ver pedido →</button>}
          </div>
        </section>
      )}

      <button style={S.secondaryWide} onClick={generateQuote} disabled={saving}>
        {saving ? "Processando..." : linkedQuote ? "📄 Ver orçamento desta oportunidade →" : "📄 Gerar Orçamento com esses itens →"}
      </button>
    </div>}
