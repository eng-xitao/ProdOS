...
export default function NotasFiscaisPageV2() {
  ...
  async function load() {
    ...
    const [{ data: r, error: re }, { data: inv }] = await Promise.all([
      supabase.from("sales_order_fulfillment_requests")
        .select("id, sales_order_id, status, requested_at, billing_requested_at, sales_orders:sales_order_id(code, customer_id, customers:customer_id(name, document))")
        .in("status", ["aguardando_faturamento", "faturado"])
        .order("billing_requested_at", { ascending: false }),
  ...
  async function loadDetail(id) {
    ...
    const [{ data: r }, { data: it }] = await Promise.all([
      ...
      supabase.from("sales_order_fulfillment_items").select("id, product_id, requested_quantity, checked_quantity, products:product_id(sku,name,unit,ncm)").eq("fulfillment_request_id", id),
    ]);
  ...
  const selectedInvoice = detail ? invoices.find(i => i.sales_order_id === detail.sales_order_id && ["processando","autorizado","erro"].includes(i.status)) : null;
  ...
  async function emit() {
    ...
    if (data?.invoice?.status === "autorizado") {
      ...
      setMessage("NF-e enviada para processamento. A solicitação permanece controlada até a confirmação da autorização.");
    }
